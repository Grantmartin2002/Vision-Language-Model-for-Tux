...
def extract_kart_objects(
    ...
    Args:
        ...
        min_box_size: Minimum width or height of a box to be considered
    ...
    if kart_objects:
        ...
        centers = np.array([obj["center"] for obj in kart_objects])
...
def generate_qa_pairs(info_path: str, view_index: int, img_width: int = 150, img_height: int = 100) -> list:
    ...
    # 4. Relative position questions for each kart (except ego)
    ...
    # 5. Counting questions
    left_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][0] < ego_x)
    right_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][0] > ego_x)
    front_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][1] < ego_y)
    behind_count = sum(1 for k in kart_objects if not k["is_center_kart"] and k["center"][1] > ego_y)
...
"""
def generate_all_qa_pairs(base_folder):
